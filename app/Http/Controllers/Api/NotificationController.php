...
use App\Traits\ApiResponseTrait; use Illuminate\Http\Request;
class NotificationController { use ApiResponseTrait;
 ...
 public function markAsRead(Request $r,string $id){$n=$r->user()->notifications()->findOrFail($id);$n->markAsRead();return $this->success($n,'Notification marked as read');}
 ...
 public function destroy(Request $r,string $id){$r->user()->notifications()->whereKey($id)->firstOrFail()->delete();return $this->success(null,'Notification deleted');}
}
